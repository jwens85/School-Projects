...
print(f"GAN {version} - Dynamic Learning Rate Training")
...
# DEBUG: Visualize real ship images after preprocessing
print("\nDEBUG: Visualizing preprocessed real ship images...")
...
plt.close()
print("Saved debug_real_ships.png")
...
def make_discriminator():
    """Discriminator with enhanced architecture to match generator complexity"""
    model = Sequential()

    # First conv block: 32x32 -> 16x16 (more filters)
    model.add(Conv2D(64, kernel_size=4, strides=2,
                     input_shape=image_shape, padding="same"))
    model.add(LeakyReLU(alpha=0.2))
    model.add(Dropout(0.25))

    # Second conv block: 16x16 -> 8x8 (increased capacity)
    model.add(Conv2D(128, kernel_size=4, strides=2, padding="same"))
    model.add(BatchNormalization(momentum=0.8))
    model.add(LeakyReLU(alpha=0.2))
    model.add(Dropout(0.25))

    # Third conv block: 8x8 -> 4x4 (more filters)
    model.add(Conv2D(256, kernel_size=4, strides=2, padding="same"))
    model.add(BatchNormalization(momentum=0.8))
    model.add(LeakyReLU(alpha=0.2))
    model.add(Dropout(0.25))

    # Fourth conv block: 4x4 -> 2x2 (increased capacity)
    model.add(Conv2D(512, kernel_size=4, strides=2, padding="same"))
    model.add(BatchNormalization(momentum=0.8))
    model.add(LeakyReLU(alpha=0.2))
    model.add(Dropout(0.25))

    # NEW: Fifth conv block for finer detail detection: 2x2 -> 1x1
    model.add(Conv2D(1024, kernel_size=2, strides=1, padding="same"))
    model.add(BatchNormalization(momentum=0.8))
    model.add(LeakyReLU(alpha=0.2))
    model.add(Dropout(0.25))

    # Enhanced dense layers for better feature evaluation
    model.add(Flatten())
    model.add(Dense(512))
    model.add(LeakyReLU(alpha=0.2))
...
# V3.4 KEY IMPROVEMENTS: Enhanced discriminator + Dynamic learning rates
DISCRIMINATOR_LR_START = 0.0002  # More conservative start
GENERATOR_LR_START = 0.0002  # Back to proven rate
LR_DECAY_FACTOR = 0.998  # Slower decay

print(f"\n{version} Training Configuration:")
print(f"- Enhanced discriminator architecture with 5 conv layers + dense layers")
print(f"- Dynamic Learning Rates: Conservative approach")
print(f"- Discriminator LR Start: {DISCRIMINATOR_LR_START}")
print(f"- Generator LR Start: {GENERATOR_LR_START}")
print(f"- LR Decay Factor: {LR_DECAY_FACTOR}")
print(f"- Fixed discriminator accuracy tracking")
print(f"- Generator training frequency: 2x per discriminator update")

# Create optimizers with initial learning rates
d_optimizer = Adam(learning_rate=DISCRIMINATOR_LR_START, beta_1=0.5)
g_optimizer = Adam(learning_rate=GENERATOR_LR_START, beta_1=0.5)

# Compile discriminator with dynamic optimizer
...
# Training metrics with separate accuracy tracking
d_losses = []
g_losses = []
d_real_accuracies = []  # Track real image accuracy separately
d_fake_accuracies = []  # Track fake image accuracy separately
...
for epoch in range(1, EPOCHS + 1):
    ...
    # V3.3 IMPROVEMENT: Dynamic learning rate decay
    current_d_lr = DISCRIMINATOR_LR_START * (LR_DECAY_FACTOR ** epoch)
    current_g_lr = GENERATOR_LR_START * (LR_DECAY_FACTOR ** epoch)

    # Update optimizer learning rates
    d_optimizer.learning_rate.assign(current_d_lr)
    g_optimizer.learning_rate.assign(current_g_lr)
    ...
    # Store metrics with separate accuracy tracking
    d_losses.append(d_loss[0])
    g_losses.append(g_loss)
    d_real_accuracies.append(d_loss_real[1])  # Real image accuracy
    d_fake_accuracies.append(d_loss_fake[1])  # Fake image accuracy

    # Print progress with learning rates
    if epoch % 100 == 0:
        avg_real_acc = d_loss_real[1]
        avg_fake_acc = d_loss_fake[1]
        print(
            f"Epoch {epoch:5d}: D_loss={d_loss[0]:.4f}, Real_acc={avg_real_acc:.3f}, Fake_acc={avg_fake_acc:.3f}, G_loss={g_loss:.4f}, D_LR={current_d_lr:.6f}, G_LR={current_g_lr:.6f}")
    ...
    if epoch % DISPLAY_INTERVAL == 0 or epoch == 1:
        ...
        # Save training metrics plot with improved accuracy tracking
        ...
        ax1.set_ylabel('Loss')
        ax1.set_title('Training Losses (Dynamic LR)')
        ax1.legend()
        ax1.grid(True, alpha=0.3)

        # Plot separate discriminator accuracies
        ...
        ax2.set_title('Discriminator Accuracies (Separate)')
        ax2.legend()
...
# Final training summary with separate accuracies
...
print(f"- Enhanced discriminator: 5 conv layers + 2 dense layers for better evaluation")
print(f"- Conservative dynamic learning rates with slower decay")
print(f"- Separate accuracy tracking for real vs fake images")
