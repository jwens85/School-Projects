# Portfolio_GAN_v3.1_Balanced_Fixed_v2.py
...
d_schedule = ExponentialDecay(0.0002, decay_steps=2500, decay_rate=0.5)
g_schedule = ExponentialDecay(0.0002, decay_steps=2500, decay_rate=0.5)
...
for epoch in range(1, EPOCHS + 1):
    ...
    real_images += np.random.normal(0, 0.01, real_images.shape)
    real_images = np.clip(real_images, -1, 1)

    noise = np.random.normal(0, 1, (BATCH_SIZE, latent_dim))
    fake_images = generator.predict(noise, verbose=0)
    fake_images += np.random.normal(0, 0.01, fake_images.shape)
