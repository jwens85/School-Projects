# Portfolio_GAN_v3.1_Balanced_Fixed.py
...
# Optimizers with learning rate schedules
d_optimizer = Adam(learning_rate=ExponentialDecay(0.0002, decay_steps=EPOCHS, decay_rate=0.5), beta_1=0.5)
g_optimizer = Adam(learning_rate=ExponentialDecay(0.0002, decay_steps=EPOCHS, decay_rate=0.5), beta_1=0.5)
...
for epoch in range(1, EPOCHS + 1):
    ...
    real_images += np.random.normal(0, 0.05, real_images.shape)
    real_images = np.clip(real_images, -1, 1)

    noise = np.random.normal(0, 1, (BATCH_SIZE, latent_dim))
    fake_images = generator.predict(noise, verbose=0)
    fake_images += np.random.normal(0, 0.05, fake_images.shape)
