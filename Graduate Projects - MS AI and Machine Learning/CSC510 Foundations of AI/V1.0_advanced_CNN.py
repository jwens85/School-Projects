...
transform = transforms.Compose([
    ...
    transforms.RandomRotation(25),  # Added rotation
    transforms.RandomPerspective(distortion_scale=0.2, p=0.5),  # Added perspective change
    transforms.RandomAffine(degrees=15, translate=(0.1, 0.1), scale=(0.9, 1.1), shear=10),
    transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.2),
    transforms.GaussianBlur(kernel_size=3, sigma=(0.1, 2.0)),  # Added blur
    transforms.ToTensor(),
...
# **Define Optimizer with Lower Initial Learning Rate for Warmup**
optimizer = optim.AdamW(model.parameters(), lr=0.00001, weight_decay=1e-4)  # Using AdamW for better regularization

# **Training Parameters**
num_epochs = 50
warmup_epochs = 5
best_accuracy = 0.0  # Track highest accuracy
...
for epoch in range(num_epochs):
    ...
    if epoch == 10:  # After 10 epochs, unfreeze layer4
        print("Unfreezing layer4")
        for param in model.layer4.parameters():
            param.requires_grad = True

    if epoch == 15:  # After 15 epochs, unfreeze layer3
        print("Unfreezing layer3")
        for param in model.layer3.parameters():
            param.requires_grad = True

    if epoch == 20:  # After 20 epochs, unfreeze layer2
    ...
    if accuracy > best_accuracy:
        ...
        }, "best_resnet152_vehicle_damage.pth")
...
torch.save({
    ...
    'accuracy': accuracy,
    'class_mapping': class_mapping
}, "final_resnet152_vehicle_damage.pth")
print("Final model saved successfully!")

# **Evaluate Best Model on Validation Set in Detail**
print("\nDetailed evaluation of best model:")

# Load the best model
checkpoint = torch.load("best_resnet152_vehicle_damage.pth")
...
for i in range(len(class_mapping)):
    ...
    print(f"{reverse_mapping[i]}: {class_acc:.2f}% ({class_correct[i]}/{class_total[i]})")
