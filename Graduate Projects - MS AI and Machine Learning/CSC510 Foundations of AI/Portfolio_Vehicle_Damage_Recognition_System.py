...
# Progressive Unfreezing Variables
...
for epoch in range(num_epochs):
    ...
    old_lr = optimizer.param_groups[0]["lr"]  # Store the current learning rate before the step
    scheduler.step(val_loss / len(val_loader))
    new_lr = optimizer.param_groups[0]["lr"]  # Get the updated learning rate if it changed

    #Print update to learning rate
    if new_lr < old_lr:
    ...
    torch.save(checkpoint, "checkpoint_resnet152_vehicle_damage_dropout.pth")
