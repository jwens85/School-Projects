...
transform = transforms.Compose([
    ...
    transforms.RandomResizedCrop(224, scale=(0.8, 1.0)),  # Randomly crop between 80% to 100% of the image
    transforms.RandomHorizontalFlip(p=0.5),  # Flip images 50% of the time
    transforms.RandomAffine(degrees=15, translate=(0.1, 0.1), scale=(0.9, 1.1), shear=10),  # Apply rotations & transformations
    transforms.ColorJitter(brightness=0.3, contrast=0.3),  # Adjust brightness & contrast randomly
    transforms.ToTensor(),
...
model = models.resnet18(weights=ResNet18_Weights.DEFAULT)  # Load pretrained ResNet18
num_ftrs = model.fc.in_features
model.fc = nn.Linear(num_ftrs, len(class_mapping))  # Replace last layer to match vehicle damage classes

# **Fine-tune last 2 ResNet layers**
...
# **Training Loop**
num_epochs = 50  # Increase epochs to fully fine-tune
...
torch.save(model.state_dict(), "resnet18_vehicle_damage_finetuned.pth")
