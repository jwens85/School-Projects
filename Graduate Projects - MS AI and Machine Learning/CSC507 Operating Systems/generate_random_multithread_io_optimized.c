...
// This is the function that generates random numbers and then writs them to a temporary file
void *generate_random_numbers(void *arg) {
    ...
    int *buffer = malloc(CHUNK_SIZE * sizeof(int)); // Memory allocation by chunk size
    if (buffer == NULL) {
