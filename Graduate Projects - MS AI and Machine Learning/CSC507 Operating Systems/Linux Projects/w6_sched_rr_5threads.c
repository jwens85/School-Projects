...
#define NUM_THREADS 5        // Number of threads changed to 5
...
int main() {
    ...
    FILE *output_file = fopen("output_w6_sched_rr_5threads.txt", "w"); // Output to a text file
    if (output_file == NULL) {
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
        ...
        while ((read_count = fread(buffer, sizeof(int), CHUNK_SIZE, temp_file)) > 0) {
            for (size_t j = 0; j < read_count; j++) {
                fprintf(output_file, "%d\n", buffer[j]); // Write as text to output_w6_sched_rr_5threads.txt
            }
        }
