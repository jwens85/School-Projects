...
#include <time.h>
#include <pthread.h>
#include <sched.h>  // For CPU affinity functions like CPU_ZERO and sched_setaffinity
...
void *generate_random_numbers(void *arg) {
    ...
    CPU_SET((int)thread_id, &cpuset); // Bind thread to a specific core
    if (sched_setaffinity(0, sizeof(cpu_set_t), &cpuset) != 0) {
        perror("Failed to set CPU affinity");
    }

    // Set real-time scheduling policy (SCHED_RR)
    struct sched_param param;
    param.sched_priority = 10;  // Priority level (1-99, higher means more important)
...
int main() {
    ...
    FILE *output_file = fopen("output_w6_sched_rr_4threads_affinity.txt", "w"); // Output to a text file
    if (output_file == NULL) {
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
        ...
        while ((read_count = fread(buffer, sizeof(int), CHUNK_SIZE, temp_file)) > 0) {
            for (size_t j = 0; j < read_count; j++) {
                fprintf(output_file, "%d\n", buffer[j]); // Write as text to output_w6_sched_rr_4threads_affinity.txt
            }
        }
