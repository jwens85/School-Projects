...
int main() {
    ...
    // Set niceness to -20 (highest priority)
    ...
    FILE *output_file = fopen("output_w6_nice_minus20.txt", "w"); // Output to a text file
    if (output_file == NULL) {
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
        ...
        while ((read_count = fread(buffer, sizeof(int), CHUNK_SIZE, temp_file)) > 0) {
            for (size_t j = 0; j < read_count; j++) {
                fprintf(output_file, "%d\n", buffer[j]); // Write as text to output_w6_nice_minus20.txt
            }
        }
