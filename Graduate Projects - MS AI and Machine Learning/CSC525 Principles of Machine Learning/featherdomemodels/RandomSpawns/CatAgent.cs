...
public class CatAgent : Agent
{
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        // Check if the agent is out of bounds and end the episode if it is
        if (Mathf.Abs(position.x) > 10f || Mathf.Abs(position.z) > 5f)
        {
            Debug.Log("CatAgent is out of bounds! Restarting episode.");
            SetReward(-1f);  // Optional penalty for going out of bounds
            EndEpisode();    // Restart the episode
            return;          // Prevent further processing if out of bounds
        }
